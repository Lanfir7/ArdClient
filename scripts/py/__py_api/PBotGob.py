...
class PBotGob(object):
    ...
    def pf_click(self, btn: int = 1, mod: int = 0, meshid: int = -1, olid: int = 0):
        self._gob.pfClick(btn, mod, meshid)
    ...
    def get_coords(self) -> Tuple[float, float]:
        c = self._gob.getRcCoords()
        return float(c.x), float(c.y),

    ## Get gob id of the gob
    # @return id
    def get_id(self) -> int:
        return int(self._gob.getGobId())
    ...
    def add_gob_text(self, text: str, height: int = 20) -> int:
        return self._gob.addGobText(text, height)
    ...
    def window_name_for_gob(self) -> str:
        return self._gob.windowNameForGob(self._gob.gob)
