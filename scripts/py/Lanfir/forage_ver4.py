...
class Script:
    def run(self, sess: PBotSession):
        # sess.close_session()
        # sess.new_session("aserne", "cd55dc16h", "Seigar")
        # self.sleep(10)
        ...
        while 1:
            ...
            points = self.read_txt("forage")
            route = points.split("\n")
            for step in route:
                if step:
                    st = step.split(",")
                    self._PBotUtils.sys_msg('Иду {}, {} [{}]'.format(int(float(st[0])), int(float(st[1])), route.index(step)))
                    walking = True
                    self._PBotUtils.map_click(float(st[0]), float(st[1]))
                    self.sleep()
                    while walking:
                        if self._PBotCharacterAPI.get_speed() == 0:
                            self._PBotUtils.sys_msg('Стоп')
                            break
                        coords = sess.PBotGobAPI.get_player().get_coords()
                        if player.is_moving() and not int(coords[0]) == int(float(st[0])) and not int(coords[1]) == int(float(st[1])):
                            ...
                            self.sleep()
                        coords = sess.PBotGobAPI.get_player().get_coords()
                        if int(coords[0]) == int(float(st[0])) and int(coords[1]) == int(float(st[1])):
                            walking = False
            #
    ...
    def read_txt(self, name):
        file = open("scripts\Lanfir\/routs/{}.txt".format(name), "r")
        filer = file.read()
        file.close()
        return filer
